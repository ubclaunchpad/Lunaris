...
describe('terminate-ec2 Lambda handler', () => {
  ...
  beforeEach(() => {
    ...
    mockDynamoDBWrapper = new DynamoDBWrapper("RunningInstances") as jest.Mocked<DynamoDBWrapper>;
  ...
  describe('error handling', () => {
    ...
    it('should keep going if DCV session stop fails', async () => {
      ...
      expect(result.message).toBe('Instance i-1234567890abcdef0 terminated successfully. DCV close-session failed: DCV stop failed'); // Use 'message' instead of 'dcvMessage'
      expect(result.detachVolumeState).toBe('detached'); 
      expect(result.terminateInstanceState).toBe('terminated');
    });
    ...
    it('should return an error if volume detachment fails', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('Failed to detach volume vol-12345. Current state: detach-failed');
    });
  });
  ...
  it('should retunr an error if DynamoDB update failure', async () => {
    ...
    // The handler should return false because DynamoDB update failed
     expect(result.success).toBe(false);
    expect(result.error).toBe('Failed to update DynamoDB for instance i-1234567890abcdef0: DynamoDB update failed');
  });
