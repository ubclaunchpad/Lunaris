...
describe("DCVWrapper", () => {
   ...
   describe("stopDCVSession", () => {
    ...
    it("should return failed result if SSM command times out or fails during wait", async () => {
        ...
        (dcvWrapper as any).waitForSSMCommand = jest.fn().mockRejectedValue(new Error("Timeout waiting for SSM command"));

        const result = await dcvWrapper.stopDCVSession();

        expect(result.stoppedSuccessfully).toBe(false);
        expect(result.message).toMatch(/DCV close-session failed: Timeout waiting for SSM command/);
    });
});
